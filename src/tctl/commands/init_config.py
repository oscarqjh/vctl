"""`tctl init-config` — scaffold cluster.yaml + models/*.yaml from canonical templates."""
...
from tctl.workloads.vllm.templates import CLUSTER_TEMPLATE, PROFILE_TEMPLATES

_DEFAULT_TARGET_DIR = str(Path.home() / ".tctl")


def _build_subparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="tctl init-config")
    p.add_argument(
        "--dir",
