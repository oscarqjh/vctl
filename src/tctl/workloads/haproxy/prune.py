"""Prune candidates collector for `tctl haproxy prune`.
...
from tctl.duration import _parse_duration  # re-exported for callers
from tctl.tmux import TmuxSession as _TmuxSession
from tctl.tmux import tmux_session_exists

# NOTE: _fetch_haproxy_stats is imported at module level so tests can monkeypatch
# at `tctl.workloads.haproxy.prune._fetch_haproxy_stats` rather than at the definition site.
...
if TYPE_CHECKING:
    from tctl.config.models import LbPrune
...
def _spawn_watcher(
    ...
    """Spawn the tctl-haproxy-watch tmux session and write the sentinel pidfile.

    Builds: while true; do python -m tctl --config <path> haproxy prune; sleep N; done
    Writes: mgr.run_dir / "watch.pid"  with content "tmux:tctl-haproxy-watch\\n"
    """
    ...
    _TmuxSession("tctl-haproxy-watch").start(["bash", "-c", loop_cmd])

    pid_path = mgr.run_dir / "watch.pid"
    pid_path.parent.mkdir(parents=True, exist_ok=True)
    tmp = pid_path.with_suffix(".tmp")
    tmp.write_text("tmux:tctl-haproxy-watch\n")
    tmp.replace(pid_path)


def _stop_watcher(mgr: LbManager) -> bool:
    """Kill the tctl-haproxy-watch tmux session and remove the sentinel pidfile.
    ...
    """
    was_running = tmux_session_exists("tctl-haproxy-watch")
    _TmuxSession("tctl-haproxy-watch").kill(tree=False)
    pid_path = mgr.run_dir / "watch.pid"
...
def _watcher_status(mgr: LbManager) -> dict[str, object]:
    """Return watcher liveness info for `tctl haproxy status` display.

    Returns dict with keys:
      - "enabled"       (bool) — mgr.lb.prune.enabled
      - "session_alive" (bool) — tmux_session_exists("tctl-haproxy-watch")
    ...
    session_alive = tmux_session_exists("tctl-haproxy-watch")
    pid_path = mgr.run_dir / "watch.pid"
    pidfile_ok = False
    if pid_path.exists():
        try:
            content = pid_path.read_text().strip()
            pidfile_ok = content == "tmux:tctl-haproxy-watch"
        except OSError:
            pass
