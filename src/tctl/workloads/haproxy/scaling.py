...
from tctl.workloads.haproxy.errors import LbUnreachable, PoolNotFound, ReconcilerError
from tctl.workloads.haproxy.manager import LbManager
from tctl.workloads.haproxy.probe import probe_local_vllm, probe_vllm
from tctl.workloads.haproxy.reconciler import Action, Reconciler
from tctl.workloads.haproxy.routing import _name_for, pool_for_endpoint, resolve_pool_ref
from tctl.workloads.haproxy.runtime import RuntimeClient, _NoOpClient
from tctl.workloads.haproxy.runtime import lb_admin_client as _client
...
# The canonical implementations live in tctl.workloads.haproxy.runtime / routing;
# they are imported above and surfaced here so:
#   1. mypy treats `from tctl.workloads.haproxy.scaling import _client` (used in
#      tctl.workloads.haproxy.commands) as an explicit re-export, not an attr-defined error.
#   2. `monkeypatch.setattr(scaling, "_client", ...)` in tests
