...
from tctl.config.models import LbHaproxy
...
_HAPROXY_SESSION_NAME = "tctl-haproxy"
_TMUX_NAME = _HAPROXY_SESSION_NAME
...
class LbManager:
    ...
    def reload(self) -> None:
        ...
        # `tctl haproxy reload` left HAProxy on the old config).
        cfg = self.render_config()
        self.cfg_path.write_text(cfg)
    ...
    def status(self) -> dict[str, object]:
        ...
        Checks (in order):
          ...
          3. tmux-managed via `tctl-haproxy` session   → ownership flag
