...
from tctl.tmux import TmuxSession, _validate_tmux_name
...
class VllmManager:
    ...
    def start(self) -> None:
        ...
        if TmuxSession(self.session_name).exists():
            raise RuntimeError(
                ...
                "use `tctl vllm restart` to restart or `tctl vllm stop` to stop."
            )
        ...
        if pid is None:
            ...
            raise RuntimeError(
                ...
                "check `tctl vllm logs` for startup errors."
            )
    ...
    def stop(self) -> None:
        ...
        # Poll for pid exit up to TCTL_KILL_GRACE.
        pid: int | None = None
        if self.pid_path.exists():
            try:
    ...
    def console(self) -> None:
        ...
        Renamed from attach() in v0.5.1 to avoid CLI confusion with `tctl haproxy attach`
        ...
        if not TmuxSession(self.session_name).exists():
            raise RuntimeError(
                ...
                "Start vllm first with `tctl vllm`."
            )
    ...
    def _rolling_restart_session_path(self, pool: str, state_dir: Path | None = None) -> Path:
        """Return ~/.tctl/vllm/rolling-restart/<pool>.json (or under state_dir if given).

        Default base: Path.home() / ".tctl" / "vllm" / "rolling-restart"
        Creates the base directory if missing.
        """
        base = (
            state_dir
            if state_dir is not None
            else Path.home() / ".tctl" / "vllm" / "rolling-restart"
        )
