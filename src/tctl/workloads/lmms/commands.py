...
def _cmd_run_loop(ns: argparse.Namespace, _argv_rest: list[str] | None = None) -> int:
    if TmuxSession(_TMUX_NAME).exists():
        print(
            f"tmux session {_TMUX_NAME!r} already exists. "
            f"attach: tmux attach -t {_TMUX_NAME}  |  kill: tctl lmms stop",
            file=sys.stderr,
        )
        return 4
...
def _cmd_stop(ns: argparse.Namespace, _argv_rest: list[str] | None = None) -> int:
...
def _cmd_status(ns: argparse.Namespace, _argv_rest: list[str] | None = None) -> int:
    if TmuxSession(_TMUX_NAME).exists():
...
def register_all(sub: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    """Register run-loop, stop, status sub-parsers onto *sub*."""
...
def run(ns: argparse.Namespace, argv_rest: list[str]) -> int:
    ...
    handler: object = parsed._handler
    return int(handler(parsed, []))  # type: ignore[operator]
