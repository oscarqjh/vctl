...
def test_pretty_logging_writes_to_stderr(capsys) -> None:
    ...
    logging.getLogger("tctl.test").info("hello pretty")
...
def test_json_logging_emits_valid_json(capsys) -> None:
    ...
    logging.getLogger("tctl.test").info("hello", extra={"event": "smoke"})
...
def test_log_level_threshold(capsys) -> None:
    ...
    logging.getLogger("tctl.test").info("should not appear")
    logging.getLogger("tctl.test").warning("should appear")
    err = capsys.readouterr().err
