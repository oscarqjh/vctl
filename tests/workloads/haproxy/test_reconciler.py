"""Unit tests for tctl.workloads.haproxy.reconciler — all RuntimeClient calls are mocked."""
...
from tctl.config.models import LbAdmin, LbHaproxy, LbStats, Pool
from tctl.workloads.haproxy.errors import PoolNotFound
from tctl.workloads.haproxy.manager import LbManager
from tctl.workloads.haproxy.reconciler import Action, Drift, Outcome, Reconciler
...
def _concurrent_want_present_worker(
    ...
    Sets TCTL_TEST_NO_SOCKET=1 in the spawned process so haproxy admin
    ...
    """
    os.environ["TCTL_TEST_NO_SOCKET"] = "1"
...
def test_want_present_raises_backend_op_failed_and_leaves_state_untouched(
    ...
    import tctl.workloads.haproxy.reconciler as reconciler_mod
    from tctl.workloads.haproxy.errors import BackendOpFailed
...
def test_concurrent_want_present_same_ep_produces_one_state_entry(
    ...
    Uses TCTL_TEST_NO_SOCKET=1 so haproxy calls are no-ops (_NoOpClient).
