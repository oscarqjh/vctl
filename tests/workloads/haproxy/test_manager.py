...
from tctl.config.models import LbAdmin, LbClient, LbDefaults, LbHaproxy, LbHealth, LbStats
from tctl.workloads.haproxy.manager import LbManager, _verify_pid_is_haproxy
...
@patch("tctl.workloads.haproxy.manager.detect_self_ip", return_value="10.0.0.1")
@patch("tctl.tmux.tmux_session_exists", return_value=False)
@patch("tctl.workloads.haproxy.manager.socket.create_connection", side_effect=OSError)
@patch("tctl.workloads.haproxy.manager._verify_pid_is_haproxy", return_value=True)
...
@patch("tctl.tmux.tmux_session_exists", return_value=False)
@patch("tctl.workloads.haproxy.manager.socket.create_connection", side_effect=OSError)
@patch("tctl.workloads.haproxy.manager.TmuxSession")
@patch("tctl.workloads.haproxy.manager.ensure_haproxy", return_value="/usr/bin/haproxy")
@patch("tctl.workloads.haproxy.manager._verify_pid_is_haproxy", return_value=True)
...
def test_stop_sigkill_fallback(
    ...
    monkeypatch.setattr("tctl.workloads.haproxy.manager.os.kill", fake_kill)
    monkeypatch.setattr("tctl.workloads.haproxy.manager.time.monotonic", fake_monotonic)
    monkeypatch.setattr("tctl.workloads.haproxy.manager.time.sleep", lambda _: None)
    monkeypatch.setattr("tctl.workloads.haproxy.manager.TmuxSession", MagicMock())
...
def test_stop_unlinks_sock_path(
    ...
    monkeypatch.setattr("tctl.workloads.haproxy.manager.os.kill", fake_kill)
    monkeypatch.setattr("tctl.workloads.haproxy.manager.time.monotonic", _real_monotonic)
    monkeypatch.setattr("tctl.workloads.haproxy.manager.time.sleep", lambda _: None)
    monkeypatch.setattr("tctl.workloads.haproxy.manager.TmuxSession", MagicMock())
...
def test_reload_raises_on_config_syntax_error(
    ...
    monkeypatch.setattr("tctl.workloads.haproxy.manager.ensure_haproxy", lambda: "/fake/haproxy")
    ...
    monkeypatch.setattr("tctl.workloads.haproxy.manager.subprocess.run", fake_run)
