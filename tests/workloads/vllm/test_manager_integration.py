"""Integration tests for VllmManager (tctl) — require real tmux + a vllm stub server.
...
- Writable ~/.tctl/vllm/ directory (created by VllmManager.__init__)
"""
...
def test_at1_detached_start_survives_caller_exit(
    ...
    """AT-1: vllm process stays alive after the calling tctl process exits.

    Spawns `tctl vllm` as a subprocess in its own session (start_new_session=True),
    ...
    # Spawn tctl vllm in its own process session.
    proc = subprocess.Popen(
        ["python", "-m", "tctl", "--profile", "test", "vllm", "--skip-preflight"],
        env=env,
    ...
    try:
        ...
        assert rc == 0, f"tctl vllm exited non-zero: {rc}"
        ...
        pid_path = Path.home() / ".tctl" / "vllm" / "test.pid"
        ...
        for f in ("test.pid", "test.cmd.json", "test.host"):
            (Path.home() / ".tctl" / "vllm" / f).unlink(missing_ok=True)
...
def test_at5_console_session_and_detach(
    ...
    (the unit test in test_manager.py covers the mock; this integration
...
def test_at6_logs_tail_and_stream(
    ...
    The unit tests in test_manager.py cover the mock path.
